...
//  Roast
//
//  Created by Xiang Li on 2017-06-29.
...
protocol RoastCellDelegate: class {
...
class RoastCell: UICollectionViewCell {
    
    weak var delegate: RoastCellDelegate?
        ...
        didSet {
            ...
            if let roast = roast {
                ...
                DispatchQueue.main.async {
                    self.userProfileImageView.loadImage(urlString: roast.user.profileImageUrl, showIndicator: true)
                    self.roastImageView.loadImage(urlString: roast.roastImageUrl, showIndicator: true)
                    //self.titleLabel.attributedText = NSAttributedString(string: roast.title, attributes: self.titleAttributes)
                    self.usernameLabel.text = roast.user.username
                    self.creationDateLabel.text = roast.creationDate.timeAgoDisplay()
                    self.likeButton.setImage(roast.hasLiked ? #imageLiteral(resourceName: "like_selected") : #imageLiteral(resourceName: "like_unselected"), for: .normal)
    ...
    let roastImageView: CustomImageView = {
    ...
    override init(frame: CGRect) {
        ...
        DispatchQueue.main.async {
            ...
            self.addSubview(self.roastImageView)
    ...
    fileprivate func setupHeader() {
        ...
        //setup optionbutton
        optionsButton.anchor(top: topAnchor, left: nil, bottom: roastImageView.topAnchor, right: rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 44, height: 0)
    ...
    fileprivate func setupPostImageView() {
        ...
        self.roastImageView.anchor(top: self.userProfileImageView.bottomAnchor, left: self.leftAnchor, bottom: self.stackView.topAnchor, right: self.rightAnchor, paddingTop: self.vertPadding, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 0, height: 0)
        roastImageView.backgroundColor = UIColor.white
    }
    
    fileprivate func setupTitleView() {
        addSubview(titleView)
        titleView.anchor(top: nil,
                         left: roastImageView.leftAnchor,
                         bottom: roastImageView.bottomAnchor,
                         right: roastImageView.rightAnchor,
                         paddingTop: 0,
                         paddingLeft: 0,
                         paddingBottom: 0,
                         paddingRight: 0,
    ...
    fileprivate func setupActionButtons() {
        ...
        bookmarkButton.anchor(top: roastImageView.bottomAnchor, left: nil, bottom: nil, right: rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 40, height: 50)
        
        commentButton.addTarget(self, action: #selector(handleComment), for: .touchUpInside)
        likeButton.addTarget(self, action: #selector(handleLike), for: .touchUpInside)
        //sendMessageButton.addTarget(self, action: #selector(), for: .touchUpInside)
        //bookmarkButton.addTarget(self, action: #selector(), for: .touchUpInside)
    }
    
    //MARK: - Handle user interaction methods
    
    func showUserProfilePage() {
        guard let uid = roast?.user.uid else {return}
        delegate?.roastCellDidTapProfileImageViewOf(userID: uid)
    }
    
    func handleComment() {
        guard let roast = roast else {return}
        delegate?.roastCellDidTapCommentButton(roast: roast)
    }
    
    func handleLike() {
        delegate?.roastCellDidTapLikeButton(cell: self)
        guard let roast = roast else {return}
        guard let uid = FIRAuth.auth()?.currentUser?.uid else {return}
        let roastID = roast.roastId
        
        if roast.hasLiked {
            FIRDatabase.database().reference().child(FBKeyString.likes).child(roastID).child(uid).removeValue(completionBlock: { (error, ref) in
                if let error = error {
                    print("*** Failed to unlike roast: \(error)")
                    return
                }
                
                roast.hasLiked = !roast.hasLiked
                ...
                print("*** Successfully \(roast.hasLiked ? "liked" : "unliked") roast")
            })
        } else {
            let values = [uid: 1]
            FIRDatabase.database().reference().child(FBKeyString.likes).child(roastID).updateChildValues(values) { (error, ref) in
                
                if let error = error {
                    print("*** Failed to like roast: \(error)")
                    return
                }
